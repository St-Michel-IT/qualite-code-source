...
>>>>>>> iss53:index.html
    ...
    \begin{frame}[fragile]
        \frametitle{Git, les actions et leurs commandes}
        \framesubtitle{Patchning avec \lstinline{git rebase}\footnote{\label{progitmerge}Git Commands - Patching, \url{https://git-scm.com/book/en/v2/Appendix-C:-Git-Commands-Patching}}}
        \transdissolve
...
...
    ...
    \begin{frame}
        \frametitle{Git, les actions et leurs commandes}
        \framesubtitle{Patchning avec \lstinline{git rebase}\cref{progitmerge}}
        \transdissolve
        \begin{itemize}
    ...
    \begin{frame}
        ...
        \transdissolve
        Que peut-on faire sur comme workflow sur le repository de ce cours (\url{https://github.com/St-Michel-IT/qualite-code-source/})~?
    \end{frame}

    \begin{frame}[fragile]
        \frametitle{Exemple de CI/CD}
        \framesubtitle{Exemple de worflow GitHub de ce repository{\footnote{Worflow de génération des documents à partir de Latex, \url{https://github.com/St-Michel-IT/qualite-code-source/blob/master/.github/workflows/build-pdf.yml}}}}
        \transdissolve
        Le workflow est dans le fichier \lstinline{.github/workflows/build-document.yml}.
        \begin{lstlisting}[language=sh]
name: Build LaTeX document
on: [ push ] # Trigger(s) du workflow, ici push uniquement
jobs:
  build_latex: # Name of the job
    runs-on: ubuntu-latest # type de machine sur laquelle les les steps vont être exécutés
    steps:
      - name: Install Lualatex # Name of the step
        run: sudo apt-get install texlive-luatex
      - name: Build PDF
        run: lualatex -file-line-error -interaction=nonstopmode -synctex=1 -output-format=pdf -output-directory=doc/build/pdf Qualité-Code-Source-L3-CSI.tex
      - name: Upload PDF
        uses: actions/upload-artifact@v2 # Utilisation d'une action pour uploader un artefact
        with:
          name: pdf
          path: doc/build/pdf/Qualité-Code-Source-L3-CSI.pdf # Chemin de l'artefact à stocker to the artifact
        \end{lstlisting}
    \end{frame}
    ...
    \begin{frame}
        ...
        \bigbreak
        Quelles sont donc les solutions pour éviter les pièges des plateformes comme GitHub, Circle CI, Jenkins, etc~?
        \pause
        \begin{itemize}
            \item Éviter des fonctionnalité propres à ces plateformes.
            \item Écrire des scripts qui peuvent être exécutés en local.
            C'est à dire agnostiques aux plateforme.
            Par exemple (scripts BASH,scripts PowerShell,scripts Gradle, Dockerfiles, scripts Ansible)
        \end{itemize}
    \end{frame}
\end{document}
